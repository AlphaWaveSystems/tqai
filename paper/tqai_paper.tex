...
Across 12 model variants ($0.5$B--$14$B parameters) including Gemma~4, \texttt{tqai} achieves \textbf{zero perplexity degradation} ($\Delta\text{ppl} = 0.00$ across every model and every quantization config tested) and produces an \textbf{80\% reduction in compressed-storage KV cache size} (compressed indices vs bf16 KV tensors, useful for serialized cache / wire format / future fused dequant-attention kernels).
On compute-bound models ($7$B+), throughput retention reaches \textbf{98--101\%} of uncompressed baseline.
We report \textbf{three principled negative results}: (a)~chunked attention via Python loops loses decisively to MLX's fused \texttt{mx.fast.scaled\_dot\_product\_attention} on Apple Silicon ($3$--$5\times$ slower at 16K--32K context, with bit-identical output --- the implementation remains shipped for CUDA users without FlashAttention, but should not be enabled on MLX); (b)~DiTFastAttn CFG sharing on MLX is functionally correct but does not deliver wall-clock speedup because the per-attention Python dispatch overhead approximately cancels the saved attention compute on Apple Silicon's already-fast fused attention kernel; and (c)~K4/V2 KV quantization \emph{does not save peak runtime memory on Apple Silicon today} --- measured peak via \texttt{mx.get\_peak\_memory()} on Qwen~2.5-7B at 4K--128K context shows \texttt{tqai} within $\pm 0.2$~GB of baseline at every length, because the v0.3.1 incremental cache strategy maintains a persistent dequantized buffer at full input precision (the design choice that enables $O(1)$ per-token decode throughput). The runtime memory unlock requires a fused dequant-attention Metal kernel (KVQuant approach) which is on the v0.5 roadmap. The total system spans 484 tests (zero regressions on the 293 pre-v0.4 tests), 12 of 13 referenced compression papers, and reproducible benchmarks for LLM throughput, video generation, long-context KV memory, and few-step video presets.
\end{abstract}
...
We also report \textbf{three principled negative results} that we believe are worth more than the wins, because they save future contributors from dead ends:
...
\begin{itemize}
  ...
  \item \textbf{K4/V2 KV quantization does not save peak runtime memory on Apple Silicon today.} Measured peak via \texttt{mx.get\_peak\_memory()} on Qwen 2.5-7B (8-bit weights, $\sim 8.7$~GB model footprint) at four context lengths spanning $32\times$:
  \begin{center}
  \small
  \begin{tabular}{rrrrr}
  \toprule
  Context & Baseline KV & tqai K4/V2 KV & $\Delta$ \\
  \midrule
  ...
  \end{tabular}
  \end{center}
  Negative $\Delta$ = \texttt{tqai} uses more memory. The pattern is consistent and structural: the v0.3.1 \texttt{incremental} cache strategy maintains a persistent dequantized buffer at full input precision (\texttt{bf16} for Qwen 7B Q8) in addition to the compressed indices, because that buffer is what enables the $O(1)$ per-token decode throughput recovery. The 80\% size reduction is real for the \emph{compressed-storage form} (compressed indices vs \texttt{bf16} KV), useful for serialized cache / wire format / future fused dequant-attention kernels. It does not show up in peak runtime memory because the buffer dominates the footprint and exists alongside the compressed storage. The runtime memory unlock requires a fused dequant-attention Metal kernel (the KVQuant approach, cited in \texttt{cache/mlx.py:9}) which is on the v0.5 roadmap and listed as future work below. Cross-validated with two independent measurement methodologies (subprocess + \texttt{mlx\_lm.generate} and direct \texttt{model(ids, cache=cache)} with the cache held alive); both agree within $\pm 0.1$~GB. Full benchmark in \texttt{benchmarks/benchmark\_kv\_memory.py} and writeup in \texttt{reports/kv\_memory\_finding.md}.
\end{itemize}

The unifying insight from all three negative results: on Apple Silicon, \emph{the fused-kernel baseline is hard to beat from Python}. \texttt{mx.fast.scaled\_dot\_product\_attention} is internally tile-blocked and threadgroup-optimized; replacing or augmenting it with a Python loop loses dispatch overhead. Similarly, the v0.3.1 incremental cache trades runtime memory for throughput by holding a dequantized buffer that the model expects in full precision. The pattern is the same in both cases: \emph{anything that needs to fire inside the attention computation, or that needs to influence what tensors the model holds during attention, requires a fused kernel that controls both phases together}. The pipeline cost of any new \texttt{scorer} or \texttt{strategy} that fires per-layer (not per-attention-call) is negligible because the framework itself is zero-overhead when unused and amortizes across the relatively cheap pre/post-attention path.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Fused dequant-attention Metal kernel} (the actual unlock for runtime memory savings, replacing the obsolete ``long-context KV memory benchmark'' future-work item which has now been done in Section~10.7 and revealed that the win does not exist without the kernel). Compute $Q \cdot K^\top$ directly on \texttt{uint8} indices + per-row $L_2$ norms + the rotation matrix, without materializing the full \texttt{bf16} K tensor. KVQuant ships this on CUDA; on Apple Silicon it would need a custom Metal shader. Estimated effort: 1--2 weeks for a working prototype, longer for parity with \texttt{mx.fast.scaled\_dot\_product\_attention}. Until this exists, \texttt{tqai}'s runtime memory profile is identical to baseline and the K4/V2 win is purely a quality / compressed-storage win.
  \item \textbf{[DONE in v0.4] Offline gradient-based Fisher calibration mode.} Shipped as \texttt{tqai.optimization.calibrate\_fisher()} + \texttt{tqai.scorers.fisher\_static.FisherStaticScorer} + \texttt{tqai calibrate} CLI command. The calibration walks the model's K/V projection modules, runs forward+backward passes on a small calibration set ($8$--$32$ prompts is enough), accumulates the squared gradients per layer, normalizes them, and saves a JSON file. The static scorer loads the JSON at runtime and serves precomputed scores via constant-time lookup. End-to-end validated on Qwen 2.5-0.5B (24~layers, 4~samples, $0.5$~seconds wall-clock, $32\times$ ratio between most and least important layer). The runtime \texttt{fisher} proxy scorer is preserved for backward compatibility but documented as broken. 17 new tests cover the calibration logic, the JSON round-trip, and the static scorer with all four \texttt{kv\_mode} variants (\texttt{k}, \texttt{v}, \texttt{max}, \texttt{mean}).
